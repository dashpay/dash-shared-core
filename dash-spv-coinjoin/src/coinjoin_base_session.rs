...
impl CoinJoinBaseSession {
    ...
    pub fn is_valid_in_outs(&self, vin: &Vec<TransactionInput>, vout: &Vec<TransactionOutput>) -> ValidInOuts {
        ...
        if vin.len() != vout.len() {
            error!(target: "CoinJoin dsf", "ERROR: inputs vs outputs size mismatch! {} vs {}", vin.len(), vout.len());
        ...
        let mut check_tx_out = |tx_out: &TransactionOutput| -> ValidInOuts {
            ...
            if denom != self.session_denom {
                error!(target: "CoinJoin dsf", "ERROR: incompatible denom {} ({}) != sessionDenom {} ({})",
            ...
            if tx_out.script_pub_key_type() != ScriptType::PayToPubkeyHash {
                error!(target: "CoinJoin dsf", "ERROR: invalid script! scriptPubKey={}", hex);
            ...
            if !set_scrip_pub_keys.insert(hex.clone()) {
                error!(target: "CoinJoin dsf", "ERROR: already have this script! scriptPubKey={}", hex);
        ...
        for tx_in in vin {
            info!(target: "CoinJoin dsf", "tx_in={:?}", tx_in);

            if tx_in.input_hash.0.is_empty() {
                error!(target: "CoinJoin dsf", "ERROR: invalid input!");
