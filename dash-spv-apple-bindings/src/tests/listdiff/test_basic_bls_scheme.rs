...
fn test_basic_bls_scheme_using_chacha() {
    ...
    let context = Arc::new(RwLock::new(FFIContext::devnet_default(
        chain,
        false,
        vec![
    ...
    let provider = FFICoreProvider::register_default(&context, ChainType::DevNet(chain));
    let mut ctx = context.write().unwrap();
    let processor = MasternodeProcessor::new(Box::new(provider));
    let result = processor.mn_list_diff_result_from_message(&message, true, 70224, &mut ctx.cache)
        .expect("failed to process diff");
    let masternode_list = &result.masternode_list;
    let bh = ctx.block_for_hash(masternode_list.block_hash)
        .map(|b| b.height)
        .unwrap_or(u32::MAX);
...
fn test_core_19_rc_2_testnet() {
    ...
    let result = processor.mn_list_diff_result_from_message(&bytes, true, 70223, &mut ctx.cache)
