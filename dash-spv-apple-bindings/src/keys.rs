...
pub unsafe extern "C" fn key_serialized_private_key_for_chain(key: *mut OpaqueKey, chain_type: ChainType) -> *mut c_char {
    ...
    ferment::FFIConversionTo::ffi_to(match *key {
...
/// # Safety
#[no_mangle]
pub unsafe extern "C" fn key_bls_serialize(key: *mut BLSKey, use_legacy: bool) -> ByteArray {
    (&*key).bls_public_key_serialized(use_legacy).into()
...
pub unsafe extern "C" fn key_ecdsa_with_bip38_key(private_key: *const c_char, passphrase: *const c_char, chain_type: ChainType) -> *mut c_char {
    ...
    let script = chain_type.script_map();
    ferment::FFIConversionTo::ffi_to_opt(ECDSAKey::key_with_bip38_key(private_key, passphrase, &script)
...
pub unsafe extern "C" fn key_ecdsa_serialized_private_key_for_chain(key: *mut ECDSAKey, chain_type: ChainType) -> *mut c_char {
    ferment::FFIConversionTo::ffi_to((&*key).serialized_private_key_for_script(&chain_type.script_map()))
}
...
pub unsafe extern "C" fn key_ecdsa_serialized_auth_private_key_for_chain(seed: *const u8, seed_len: usize, chain_type: ChainType) -> *mut c_char {
    ...
    ferment::FFIConversionTo::ffi_to(ECDSAKey::serialized_auth_private_key_from_seed(seed, script_map))
}
...
pub unsafe extern "C" fn key_serialized_extended_private_key_from_seed(
    ...
    ferment::FFIConversionTo::ffi_to_opt(ECDSAKey::serialized_extended_private_key_from_seed(secret_slice, index_path, chain_type).ok())
}
...
pub extern "C" fn ecdsa_public_key_hash_from_secret(secret: *const c_char, chain_type: ChainType) -> ByteArray {
    ...
    let private_key_string = c_str.to_str().unwrap();
    ECDSAKey::key_with_private_key(private_key_string, chain_type)
        .map_or(ByteArray::default(), |key| key.hash160().into())
}
...
pub unsafe extern "C" fn key_address_with_public_key_data(data: *const u8, len: usize, chain_type: ChainType) -> *mut c_char {
    ...
    let data = slice::from_raw_parts(data, len);
    ferment::FFIConversionTo::ffi_to(address::with_public_key_data(data, &map))
}
/// # Safety
#[no_mangle]
pub unsafe extern "C" fn address_for_ecdsa_key(key: *mut ECDSAKey, chain_type: ChainType) -> *mut c_char {
    let script_map = chain_type.script_map();
    ferment::FFIConversionTo::ffi_to((&*key).address_with_public_key_data(&script_map))
}
/// # Safety
#[no_mangle]
...
pub unsafe extern "C" fn address_for_ecdsa_key_recovered_from_compact_sig(data: *const u8, len: usize, digest: *const u8, chain_type: ChainType) -> *mut c_char {
    ...
    let script_map = chain_type.script_map();
    ferment::FFIConversionTo::ffi_to_opt(UInt256::from_const(digest).map_err(ProcessingError::from)
...
pub unsafe extern "C" fn ecdsa_address_from_public_key_data(data: *const u8, len: usize, chain_type: ChainType) -> *mut c_char {
    ...
    ferment::FFIConversionTo::ffi_to_opt(ECDSAKey::key_with_public_key_data(public_key_data)
        .ok()
...
pub unsafe extern "C" fn key_secret_key_string(ptr: *mut OpaqueKey) -> *mut c_char {
    ferment::FFIConversionTo::ffi_to(match *ptr {
