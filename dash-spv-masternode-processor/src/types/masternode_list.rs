    ...
    fn drop(&mut self) {
        unsafe {
            ...
            if !self.masternode_merkle_root.is_null() {
                unbox_any(self.masternode_merkle_root);
            }
            if !self.llmq_merkle_root.is_null() {
                unbox_any(self.llmq_merkle_root);
            }
            unbox_any_vec_ptr(self.masternodes, self.masternodes_count);
            unbox_any_vec_ptr(self.llmq_type_maps, self.llmq_type_maps_count);
        }
    }
}