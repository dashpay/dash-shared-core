...
impl Display for QRInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        let sl = if !self.quorum_snapshot_list.is_empty() { format!("snapshots:\n\t\t{}\n\t", self.quorum_snapshot_list.iter().fold(String::new(), |mut acc, q| {
            acc.push_str(format!("{q}\n\t\t").as_str());
            acc
        }))} else {String::new() };
        let dl = if !self.mn_list_diff_list.is_empty() { format!("diffs:\n\t\t{}\n\t", self.mn_list_diff_list.iter().fold(String::new(), |mut acc, q| {
            acc.push_str(format!("{q}\n\t\t").as_str());
            acc
        })) } else { String::new() };
        let desc = format!("QRInfo \
            diffs: \n\t{h4c}h-3c:\n\t{}\n\th-2c:\n\t{}\n\th-c:\n\t{}\n\th:\n\t{}\n\ttip:\n\t{}\n\t \
            snapshots\n\t\t{sh4c}h-3c: {}\n\t\th-2c: {}\n\t\th-c: {}\n\t \
            {aq}\t{sl}\t{dl}\n",
            self.diff_h_3c,
...
impl QRInfo {
    pub fn new(
    ...
    ) -> Result<QRInfo, ProcessingError> {
        let mut offset = 0;
        let block_height_lookup = |block_hash|
            processor.height_for_block_hash(block_hash);
        let read_list_diff = |offset: &mut usize|
            MNListDiff::new(message, offset, processor, protocol_version);
        let read_snapshot = |offset: &mut usize|
            LLMQSnapshot::from_bytes(message, offset)
                .map_err(ProcessingError::from);
        let read_var_int = |offset: &mut usize|
            VarInt::from_bytes(message, offset)
                .map_err(ProcessingError::from);
        ...
        let extra_share = message.read_with::<bool>(&mut offset, ()).unwrap_or(false);
        ...
        for _i in 0..quorum_snapshot_list_count {
            quorum_snapshot_list.push(message.read_with::<LLMQSnapshot>(&mut offset, byte::LE)
                .map_err(ProcessingError::from)?);
        }
...
//             processor.height_for_block_hash(block_hash);
//         let read_list_diff = |offset: &mut usize|
//             MNListDiff::new(bytes, offset, processor, protocol_version);
//         let read_snapshot = |offset: &mut usize|
